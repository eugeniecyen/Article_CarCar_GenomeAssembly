...
# Map trimmed Illumina reads to the genome assembly (Flye + Medaka + Pilon x1)
...
do
   ...
   $ASSEMBLY_DIR/turtle_flye_medaka_pilon.fasta \
   $DATA_DIR/"${i:57:37}1.fq.gz" $DATA_DIR/"${i:57:37}2.fq.gz" | samtools sort - > $BAM_DIR/"${i:57:6}_${i:81:9}_flye_medaka_pilon.sort.bam"
done
